...
この台帳は、Web版が直接または同梱ファイル経由で利用するデータ、地図、ライブラリの出所と再利用条件を追跡するためのものです。
...
| 日々の震源リスト・震央分布 | 気象庁。`https://www.data.jma.go.jp/eqev/data/daily_map/index.html`、日付別HTML | [気象庁ホームページ利用規約](https://www.jma.go.jp/jma/kishou/info/coment.html)。震源要素は暫定値であり、後日変更される場合がある。 | Workerが日付別HTMLを解析し、1日分を1つのJSON行としてD1へ最大731日（うるう年を含む24か月相当）保存。通常の公開時差に合わせてJSTの2日前を期待最新日とし、Workers FreeのCPU上限を超えないよう、初回補完は1分ごとに1日だけ取得する。直近8日、保存期間内の欠損、古い未取得日の順に補完し、731日を超えた場合だけ最古日を削除する。Web版は保持した日付を切り替え、地図には選択した1日分だけを深さ別に配色して表示する。規模・深さで絞り込み可能。1日5,000件または1.5 MBを超える日は安全のため保存しない。最終確認日: 2026-07-22。 |
| 気象庁防災情報XML（地震・津波） | [気象庁防災情報XML](https://xml.kishou.go.jp/) の地震火山フィード（`eqvol.xml`、`eqvol_l.xml`）とVXSE51/52/53・VTSE41/51/52電文。 | [気象庁ホームページについて](https://www.jma.go.jp/jma/kishou/info/coment.html)を確認。行政機関の公式アプリと誤認されない表示を行い、配信を保証しない。 | Web版がXMLフィードを取得し、履歴・震源・規模・深さ・最大震度・細分区域・観測点・津波警報等・沿岸／沖合観測をEventID単位で統合する。取得不能を「津波の心配なし」と判定せず、未確認として表示する。 |
| 気象庁 潮位観測情報 | [潮位観測情報](https://www.jma.go.jp/bosai/tidelevel/) の観測点一覧、最新時刻、日別実測潮位、年別天文潮位JSON。 | [気象庁ホームページ利用規約](https://www.jma.go.jp/jma/kishou/info/coment.html)。リアルタイム値は速報値で、機器障害や通信異常等による異常値を含む場合があるため、公式画面の注意事項を優先する。 | 地震タブに公式観測点を表示し、選択した地点の直近24時間を最大720点へ間引く。実測潮位と毎正時の天文潮位を時刻補間して要約バーのグラフに重ね、1・6・12・24時間を切り替える。津波情報の発表有無には依存しない。 |
...
| ECMWF IFS ENS・IFS HRES・AIFS ENS・AIFS Single 熱帯低気圧進路 | [ECMWF Open Data](https://www.ecmwf.int/en/forecasts/datasets/open-data) の各モデルの熱帯低気圧進路BUFR。AIFSについては[AIFS Machine Learning data](https://www.ecmwf.int/en/forecasts/datasets/aifs-machine-learning-data)も参照する。 | [Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/)。[ECMWF Standard Licence](https://apps.ecmwf.int/datasets/licences/general/)に従い「© European Centre for Medium-Range Weather Forecasts (ECMWF), Source: www.ecmwf.int」を表示し、加工済みであることを明示する。ECMWFはデータの誤り・欠落や利用結果を保証しない。 | GitHub Actionsが6時間ごとにIFS ENS、IFS HRES、AIFS ENS、AIFS SingleのBUFRを個別取得し、ecCodesで座標・予報時間・中心気圧・最大風速をモデル別JSONへ変換する。要約バーの「各国予想」内で各モデルを個別にオン／オフし、複数モデルを地図へ重ねる。アンサンブル系の熱帯擾乱は20本以上のメンバーが支持する上位12候補だけを「発達候補」として区別する。決定論モデルは1本の参考進路として表示する。各国予想は数値予報の参考情報であり、発生確率や気象庁の公式な台風進路予報ではない。 |
| NOAA/NCEP GEFS 熱帯低気圧進路・GEFSアンサンブル平均 | [NCEP ensemble tracker](https://nomads.ncep.noaa.gov/pub/data/nccf/com/ens_tracker/prod/) のGEFS TIGGE XML進路とgenesis確率製品。 | [NOAA Global Ensemble Forecast System Open Data](https://registry.opendata.aws/noaa-gefs/) は公開利用可能。NOAA/NCEPを出典表示し、加工済みであること、NOAAによる推奨・提携を意味しないことを明示する。[NWS Disclaimer](https://www.weather.gov/disclaimer)も参照する。 | GitHub Actionsが6時間ごとに最新の運用サイクルを探索し、31メンバーの進路と発生確率20%以上の候補をJSONへ変換する。Web版は同じ予報時刻の有効メンバー位置を経度の循環平均に対応して平均化し、GEFS平均を別レイヤーとして生成する。要約バーの「各国予想」内でGEFSメンバーとGEFS平均を独立してオン／オフできる。発生確率はNCEP tracker製品の値であり、台風発生の確定情報や気象庁の公式予報ではない。 |
...
| `jma-tsunami-observation-stations.json` | 気象庁「[津波データ](https://www.data.jma.go.jp/eqev/data/bulletin/tsunami.html)」の潮位観測点リスト`stat_j.txt`。沿岸の検潮所・津波観測計と沖合のGNSS波浪計・海底津波計等を含む。 | 気象庁ホームページ利用規約。観測点コード、名称、表示用座標、所属機関、津波予報区コードを利用する。 | `scripts/update-jma-tsunami-observation-stations.mjs`がShift_JIS資料を読み込み、重複コードは現行一覧側に先に現れる行を採用し、緯度経度を十進度へ変換する。VTSE51/52電文の観測点コードを優先して照合し、コードがない場合だけ重複のない完全一致地点名を使用する。座標を推測補完しない。 |
...
| `demotiles.maplibre.org` | MapLibre [demo tiles repository](https://github.com/maplibre/demotiles) | リポジトリはBSD 3-Clauseだが、READMEはデモ・hello world・CI向けのサービスと説明しており、本番SLAや恒久利用を保証していない。 | Webの文字グリフURLに残存するため、一般公開前の未解決事項。 |
