...
2. GitHub リポジトリ `Weather-viewer` を接続する
...
独自ドメインは必須ではなく、まずは `weather-viewer.pages.dev` のような Pages 標準 URL で公開できます。
...
GitHub Actions 上では `GITHUB_PAGES=true` を指定し、Vite の `base` を `/Weather-viewer/` にしてビルドします。ローカル開発時と Cloudflare Pages では `/` で動作します。
