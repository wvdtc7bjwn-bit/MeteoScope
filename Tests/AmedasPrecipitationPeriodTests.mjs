...
assert.match(panelSource, /class="amedas-precipitation-legend"/);
assert.match(panelSource, /class="legend-item"/);
assert.match(panelSource, /export function getMapLegendUnit/);
assert.match(panelSource, /return "mm\/h"/);
assert.match(panelSource, /AMEDAS_METRICS\.find\(\(item\) => item\.id === amedasMetricId\)\?\.unit/);
assert.match(panelSource, /export function formatLegendLabelWithUnit/);
assert.match(panelSource, /formatLegendLabelWithUnit\(localizeText\(label\), unit\)/);
assert.match(panelSource, /unit === "mm\/h"/);
assert.match(panelSource, /formatLegendLabelWithUnit\(localizeText\(level\.label\), "mm"\)/);
assert.doesNotMatch(panelSource, /amedas-precipitation-gradient/);
...
assert.doesNotMatch(styleSource, /\.amedas-precipitation-gradient\s*\{/);
assert.doesNotMatch(styleSource, /\.legend-unit\s*\{/);
