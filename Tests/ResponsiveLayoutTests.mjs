...
assert.match(styles, /\.radar-time-controls \.location-insight-radar\s*\{[\s\S]*?background:\s*transparent !important;[\s\S]*?box-shadow:\s*none !important;/);
...
assert.match(styles, /html\[data-theme="light"\] \.weather-chart-controls\s*\{[\s\S]*?background:\s*transparent;[\s\S]*?box-shadow:\s*none;/);
...
assert.doesNotMatch(panel, /重ねた雨雲レーダー時刻/);
assert.doesNotMatch(panel, /mobile-dock-satellite-radar-timeline/);
...
assert.match(mobileTapControls, /if \(event\.defaultPrevented\) return;/);
