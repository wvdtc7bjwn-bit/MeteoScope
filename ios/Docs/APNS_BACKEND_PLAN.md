...
Web版はWeb PushでMeteoScope管理者からのお知らせだけを配信し、警報・注意報の差分通知は行わない。iOSネイティブアプリはApple Push Notification service（APNs）を使い、地域別の警報・注意報通知を維持する。WebとiOSでは、購読保存、送信処理、端末識別子、解除処理を分離する。
...
2. 58官署は8回のcronで一巡する。cronが1分間隔なら全国収集の理論最大周期は約8分に、実行・配信時間を加えた値になる。
3. 1官署の取得失敗時は直前の正常スナップショットを残し、空配列や「警報なし」で上書きしない。
4. 一巡後の警報通知フェーズはiOS購読だけを1回6件ずつ処理する。Web版の管理者通知は官署取得フェーズだけ1回4件まで処理し、警報通知フェーズ中は警報を優先して次の取得フェーズまで延期する。外部サブリクエストは官署取得時最大19件、警報通知判定時最大7件に抑える。D1クエリも保持期間整理や無効購読削除が重なる場合を含め50件未満の余裕を取る。
