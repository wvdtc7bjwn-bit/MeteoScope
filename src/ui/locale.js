...
const STATIC_UI_TRANSLATIONS = new Map(Object.entries({
  ...
  "Web版のお知らせ通知を有効にした場合、ブラウザのPush購読情報を管理者からのお知らせ配信のために保存します。Web版では気象警報・注意報をプッシュ通知せず、通知対象区域、通知設定、警報状態を保存しません。通知解除または購読が無効になった場合に削除し、30日間利用がない購読も削除します。配信にはブラウザやOSが利用するPush配信サービスを経由します。": "When web notices are enabled, the browser push subscription is stored to deliver administrator notices. The web app does not send push weather warnings or store target areas, weather notification settings, or warning states. Subscriptions are removed when disabled or invalid, and after 30 days without app use. Delivery is handled through the push service used by the browser or OS.",
