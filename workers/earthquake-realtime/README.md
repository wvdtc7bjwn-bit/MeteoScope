...
震央分布は気象庁「日々の震源リスト」の日付別HTMLを毎時17分に1日分だけ取得し、1日分のJSONをD1の1行へまとめて保存します。地震1件ごとのINSERTは行いません。ブラウザとiOSから気象庁へ直接アクセスせず、画面閲覧をきっかけにD1へ書き込む処理もありません。取得済みの日は再取得せず、失敗時も6時間は再試行しません。保持期間は15日で、期限を超えた日別データと同期状態は自動削除します。
