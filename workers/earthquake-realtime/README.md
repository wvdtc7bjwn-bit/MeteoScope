...
- `GET /api/health`
- `GET /api/distribution?dayOffset=0..14&minMagnitude=all|0..5&maxDepth=all|30|100|300|700`

電文の投入、管理、WebSocket接続、秘密情報を扱うルートは公開Workerから転送しません。DM-D.S.S APIキーは `wrangler secret` で登録し、リポジトリへ保存しないでください。

## 震央分布

震央分布は気象庁「日々の震源リスト」の日付別HTMLを毎時17分に1日分だけ取得し、1日分のJSONをD1の1行へまとめて保存します。地震1件ごとのINSERTは行いません。ブラウザとiOSから気象庁へ直接アクセスせず、画面閲覧をきっかけにD1へ書き込む処理もありません。取得済みの日は再取得せず、失敗時も6時間は再試行しません。保持期間は15日で、期限を超えた日別データと同期状態は自動削除します。15日推移グラフは同じ日別行の`record_count`を読み取るため、グラフ専用のD1書き込みは発生しません。
...
APIは最新日を0とする`dayOffset`で日付を選び、選択された1日分だけを返します。Web/iOSは要約バーまたは地震カードから最大15日を切り替えます。API応答は5分キャッシュし、画面には気象庁の暫定値で後日変更される場合があることを表示します。
