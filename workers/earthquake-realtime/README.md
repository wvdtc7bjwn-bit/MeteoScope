...
震央分布は気象庁「日々の震源リスト」の日付別HTMLを毎日00:00（日本時間）に取得し、1日分のJSONをD1の1行へまとめて保存します。地震1件ごとのINSERTは行いません。ブラウザとiOSから気象庁へ直接アクセスせず、画面閲覧をきっかけにD1へ書き込む処理もありません。取得済みの日は再取得せず、失敗時も6時間は再試行しません。保持対象は公開済みの最新30日分です。新しい公開日を保存できた場合だけ31件目以降の最古データを削除するため、気象庁側で最新日が未公開でも古いデータだけが消えることはありません。30日推移グラフは同じ日別行の`record_count`を読み取るため、グラフ専用のD1書き込みは発生しません。
...
初回補完は1回最大15日分です。空のD1では毎日00:00のCronごとに最大15日ずつ補完します。15回の外部fetch、日別D1 batch、状態確認・清掃を合わせても、Workers Freeの1回50クエリ/サブリクエスト上限内に収めています。30日未満の状態を検出した場合は、古い未取得日を候補へ加えて不足分を優先補完します。30日分が揃うと古い日の補完fetchは0件になり、以後は保存済みの最新日より新しい未取得日だけを確認します。通常は新しい公開日を1件保存し、最古の1件を削除して30件を維持します。

APIは最新日を0とする`dayOffset`で日付を選び、選択された1日分だけを返します。Web/iOSは要約バーまたは地震カードから最大30日を切り替えます。API応答は5分キャッシュし、画面には気象庁の暫定値で後日変更される場合があることを表示します。
