...
震央分布は気象庁「日々の震源リスト」の日付別HTMLを毎時17分に取得し、1日分のJSONをD1の1行へまとめて保存します。地震1件ごとのINSERTは行いません。ブラウザとiOSから気象庁へ直接アクセスせず、画面閲覧をきっかけにD1へ書き込む処理もありません。取得済みの日は再取得せず、失敗時も6時間は再試行しません。保持期間は30日で、期限を超えた日別データと同期状態は自動削除します。30日推移グラフは同じ日別行の`record_count`を読み取るため、グラフ専用のD1書き込みは発生しません。
...
初回補完は1回最大15日分です。既存15日から30日への移行は次回の毎時Cron 1回で補完し、空のD1でも最大2回で30日分が揃います。1日15回の外部fetch、日別D1 batch、状態確認・清掃を合わせても、Workers Freeの1回50クエリ/サブリクエスト上限内に収めています。30日分が揃うと過去日の補完fetchは0件になり、以後は未取得の新しい日だけを取得します。毎時Cronは状態確認と30日超過データの削除のため継続します。
