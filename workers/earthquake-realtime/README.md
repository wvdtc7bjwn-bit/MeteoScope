...
停止時間を避ける場合は、先にこのWorkerを稼働させて `/api/health` の `dmdata.connected`、`gdEarthquakeBackfill.lastRunAt`、`historyCount` を確認してからMeteoScopeを公開します。その後にEQ-app側のDM-D.S.S接続を停止します。同一APIキーで同時接続できる数は契約条件を確認してください。
