...
初回補完は1分間隔で1回1日分です。外部HTMLの解析を1件に制限し、Workers FreeのCPU上限超過を防ぎます。通常の公開時差に合わせてJSTの2日前を期待最新日とし、直近8日の未保存日を最優先し、次に保存期間内の欠損、最後に古い未取得日を補完します。保存日数と最古・最新日だけをD1側で集計するため、毎分731行をWorkerへ読み込むことはありません。731日分が揃い期待最新日も保存済みになると外部fetchとD1書き込みを停止し、日付ごとの完了状態を6時間キャッシュします。新しい公開日を保存して731件を超えた場合だけ最古の1件を削除します。同期結果は「取得失敗」「保存欠落」「気象庁公開待ち」を分けてAPIとWeb/iOSへ返します。
